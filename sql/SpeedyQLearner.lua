...
function sql:reset(state)
    if not state then
        return
    end
    self.best_network = state.best_network
    self.network = state.model
    self.networkPrev = state.model
    self.w, self.dw = self.network:getParameters()
    self.dw:zero()
    self.numSteps = 0
...
function sql:getQUpdate(args)
    ...
    q2_prev = q2_prev_max:clone():mul(self.discount):cmul(term)
...
function sql:sample_validation_data()
...
function sql:compute_validation_statistics()
...
function sql:perceive(reward, rawstate, terminal, testing, testing_ep)
...
function sql:eGreedy(state, testing_ep)
